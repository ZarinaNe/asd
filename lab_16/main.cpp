...
struct thing{
    std::string name;
    int weight, price;
    thing(std::string  name,int weight, int price): name(name), weight(weight), price(price) {}
};
 ...
 int CountMaxPrice(thing T [N], int maxCapacity) {// Возвращает максимальную стоимость вещей, которые можно получить в рюкзак вместимости maxCapacity
    ...
    for (int i = 0; i <= N; i++) {// Проходим по всем вещам
        for (int j = 0; j <= maxCapacity; j++) {// Проходим по всем "малым" рюкзакам
            ...
            if (i == 0 || j == 0) {
                arr[i][j] = 0;
            } else {// Попали в рюкзак
                if (T[i - 1].weight > j) {// если вес текущей вещи больше максимальной вместимости i-го рюзкака
    ...
    while (arr[k][s]!= 0) {
        if (arr[k - 1][s] != arr[k][s]) {
            std::cout << T[k-1].name << ": вес " << T[k-1].weight << "ед., цена: " << T[k-1].price << "\n";
            k = k - 1;
            s = s - W[k];
        } else {
            k = k -1;
...
int main()
{
     std::cout << "Лабораторная работа №16 \nРешить дискретную задачу о рюкзаке." << std::endl;
     // Набор вещей
    ...
    std::cout << "Максимальная стоимость товаров = " << maxPrice << " для рюказка вместимости " << maxCapacity << "ед." << std::endl;
    return 0;
}
